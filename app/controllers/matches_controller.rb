class MatchesController < ApplicationController
  ...
  def index 
    @competition = Competition.find(23) # FIXME (also hardcoded in query in show!!!!
  ...
  def show    
    ...
    if @match.scheduled_at.nil? 
      if user_signed_in?
        teams = Team.joins(:competitions, :team_participations).where("user_id = ? and role = ? and competitions.id = ? and (teams.id = ? or teams.id = ?)", current_user.id, 0, 23, @match.team1.id, @match.team2.id) # FIXME: hardcoded competition!!!!
        if teams.empty?
          @managed_team = nil
        else
